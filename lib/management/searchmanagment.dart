...
import 'onest.dart';
class SearchManagment extends StatefulWidget{
  final info;
  SearchManagment(this.info);
  @override
  State<StatefulWidget> createState() {
    return SearchManagmentState();
  }
}

class SearchManagmentState extends State<SearchManagment>{
  String name = "";
  @override
      ...
      child: Directionality(
          ...
          child: Scaffold(
            ...
            body: StreamBuilder<QuerySnapshot>(
              stream: (name != "")
                  ? Prov.users.orderBy("name").startAt([name]).endAt([name + '\uf8ff']).snapshots(includeMetadataChanges: true)
                  : Prov.users.snapshots(includeMetadataChanges: true),
